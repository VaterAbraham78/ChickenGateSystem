...
/***	Tastereingabe via Touch-HMI-Button		***/
...
const byte NEX_CID_MAXLIGHTTIME = 6;		// effektive Nextion-ID Eingabefeld "max.Einschaltdauer Licht Stall"
...
const char NEX_NAME_MAXLIGHTTIME[] = "pageBoxlight.p2_nb06";	// effektiver Objektname Eingabefeld "max.Einschaltdauer Licht Stall"
...
void nexFrameAuswerten(byte* frame, byte len)	{
	...
	if ((frame[0] == 0x65) && (len >= 4))	{								// Touch-Ereignis: 0x65, page, component, event
		...
		if (eventType == 0x00)	{											// Nur bei "Loslassen" reagieren (Wert steht dann fest)
			...
			else if ((pageId == NEX_PAGE_BOXLIGHT) && (compId == NEX_CID_MAXLIGHTTIME))	{ req = HMI_REQ_MAXLIGHTTIME;	name = NEX_NAME_MAXLIGHTTIME;	}
